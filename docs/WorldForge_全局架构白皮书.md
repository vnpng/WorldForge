...
# WorldForge 全局技术白皮书 (当前版本: V14.2.1)

## 一、 项目基础信息
* **项目名称**: WorldForge
* **当前版本**: V14.2.1 (Docker Support & Advanced Tuning)
* **项目定位**: 基于 FastAPI + SQLite 的模块化文字冒险游戏引擎与聊天终端。已支持基于 Docker 和 docker-compose 的生产环境容器化部署（基于 FastAPI + Uvicorn 运行）。
* **核心技术栈**:
...
├── docker-compose.yml   # 容器编排配置（包含端口映射与数据卷挂载）
...
3. **`Sessions` 表 (会话与存档)**
   * 字段: `id` (TEXT PK), `user_id` (TEXT FK), `type` (TEXT), `title` (TEXT), `messages` (TEXT), `updatedAt` (INTEGER), `is_pinned` (INT), `engine_id` (TEXT), `world_id` (TEXT), `char_id` (TEXT)。

4. **`SystemPrompts` 表 (引擎预设)**
   * 字段: `id` (TEXT PK), `user_id` (TEXT FK/system), `name` (TEXT), `intro` (TEXT), `content` (TEXT), `is_public` (INT), `sort_index` (INT)。
...
6. **`Worlds` 表 (世界资产) [V14 NEW]**
   * 字段: `id` (TEXT PK), `user_id` (TEXT FK), `name` (TEXT, 必填), `intro` (TEXT, 必填), `desc` (TEXT), `conflict` (TEXT), `society` (TEXT), `history` (TEXT), `geography` (TEXT), `magic_system` (TEXT), `rules` (TEXT), `extra_rules` (TEXT), `sort_index` (INT), `is_public` (INT), `created_at` (INT)。
   * 必填项: `name`, `intro`。
...
7. **`Characters` 表 (角色资产) [V14 NEW]**
   * 字段: `id` (TEXT PK), `user_id` (TEXT FK), `name` (TEXT, 必填), `gender` (TEXT, 必填), `age` (TEXT, 必填), `race` (TEXT), `identity` (TEXT, 必填), `appearance` (TEXT), `personality` (TEXT), `item` (TEXT), `style` (TEXT), `custom` (TEXT), `sort_index` (INT), `is_public` (INT), `created_at` (INT)。
   * 必填项: `name`, `gender`, `age`, `identity`。
   * 表单折叠规则: `name / gender / age / race / identity` 默认展开；其余字段默认折叠。

8. **`SystemConfig` 表 (系统元数据)**
   * 字段: `key` (TEXT PK), `value` (TEXT)。
   * 说明: 用于记录系统级配置与数据库版本控制（如 `db_version`）。
...
### 7. 会话置顶与拖拽排序协议
* 会话列表采用双字段排序逻辑：优先按照 `is_pinned DESC`（置顶状态）排序，其次按照 `updatedAt DESC`（更新时间）排序。
* 资产列表（引擎/世界/角色）拖拽完成后，前端通过调用对应的 RESTful 批量更新接口（如 `/api/prompts/reorder`）将新的 `sort_index` 全量持久化至云端。

### 8. 数据资产备份与迁移机制
* 系统支持对会话存档、世界设定、角色设定、API 节点进行 JSON 格式的全量导出与导入。导入时通过前端解析 JSON 并配合 `genNewId()` 重新生成主键，经由对应的 `POST` 接口逐条写入数据库，实现无损迁移。

### 9. AI 引擎高级调优参数
* **Context Limit (携带历史记忆长度)**：限制单次请求发送给大模型的对话条数，避免 Token 消耗暴涨。
* **STM Threshold (短期记忆归纳阈值)**：决定 AI 短期记忆的容量上限，超限将强制触发记忆压缩。
* **Temperature 分离机制**：为 RPG 模式（发散性推演）和 Chat 模式（严谨问答）分别设置了独立的温度控制（`rpgTemp` 与 `chatTemp`）。

---

## 五、 版本更新日志 (Changelog)

### [V14.2.1] Docker Support & Advanced Tuning [CURRENT]
* **生产环境适配**: 引入 Dockerfile 与 docker-compose.yml，支持一键容器化部署。
* **排序与置顶**: 完善会话置顶逻辑与资产拖拽排序的云端持久化协议。
* **数据迁移**: 补充全量数据（会话/世界/角色/API）的 JSON 导出导入机制。
* **调优参数**: 引入 Context Limit、STM Threshold 及 Temperature 分离控制机制。
* **版本控制**: 数据库新增 SystemConfig 表，支持版本号与元数据管理。

### [V14.0.0] UI Overhaul & Asset System (重大重构)
